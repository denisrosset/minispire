...
trait CRingFunctions[C[T] <: CRing[T]] extends CRigFunctions[C] {
  ...
  def negate[@sp(Int, Long, Float, Double) A](x: A)(implicit ev: C[A]): A =
    ev.negate(x)

  def minus[@sp(Int, Long, Float, Double) A](x: A, y: A)(implicit ev: C[A]): A =
    ev.minus(x, y)

  def fromInt[@sp(Int, Long, Float, Double) A](n: Int)(implicit ev: C[A]): A =
    ev.fromInt(n)

  def fromBigInt[@sp(Int, Long, Float, Double) A](n: BigInt)(implicit ev: C[A]): A =
    ev.fromBigInt(n)
