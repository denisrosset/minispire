...
trait CRigFunctions[C[T] <: CRig[T]] {
  ...
  def plus[@sp(Int, Long, Float, Double) A](x: A, y: A)(implicit ev: C[A]): A =
    ev.plus(x, y)

  def times[@sp(Int, Long, Float, Double) A](x: A, y: A)(implicit ev: C[A]): A =
    ev.times(x, y)

  def zero[@sp(Int, Long, Float, Double) A](implicit ev: C[A]): A =
    ev.zero

  def one[@sp(Int, Long, Float, Double) A](implicit ev: C[A]): A =
    ev.one

  def isZero[@sp(Int, Long, Float, Double) A](a: A)(implicit ev0: C[A], ev1: Eq[A]): Boolean =
    ev0.isZero(a)

  def isOne[@sp(Int, Long, Float, Double) A](a: A)(implicit ev0: C[A], ev1: Eq[A]): Boolean =
    ev0.isOne(a)

  def sumN[@sp(Int, Long, Float, Double) A](a: A, n: Int)(implicit ev: C[A]): A =
    ev.sumN(a, n)
